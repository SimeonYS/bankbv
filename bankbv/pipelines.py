...
class BbankbvPipeline:

    # Database setup
    conn = sqlite3.connect('bankbv.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bankbv`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bankbv WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bankbv (date, title, link, content)"
